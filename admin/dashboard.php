...
// Total liter
$qLiter = mysqli_query($koneksi, "SELECT SUM(volume) as total FROM transaksi");
$dLiter = mysqli_fetch_assoc($qLiter);
$totalLiter = $dLiter['total'] ?? 0;

// Total pelanggan
$qUser = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM users WHERE role != 'mitra'");
$dUser = mysqli_fetch_assoc($qUser);
$totalPelanggan = $dUser['total'] ?? 0;

// Total mitra
$qMitra = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM users WHERE role = 'mitra'");
